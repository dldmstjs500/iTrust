...
<%@ page import="edu.ncsu.csc.itrust.filters.LabProceduresFilter" %>
...
<%
    ...
    LabProceduresFilter filter;
    String filterJson;
    if (!saveFilter && !isTestingFilter) {
        filterJson = action.getLabProceduresFilter(pid);
        filter = LabProceduresFilter.fromJsonString(filterJson);
    }
    else {
        filter = LabProceduresFilter.builder()
                .priority(NumberUtils.toLong(request.getParameter("priority"), 0))
                .labProcedureId(NumberUtils.toLong(request.getParameter("lab_procedure_id"), 0))
                .lastUpdatedDateStart(request.getParameter("last_updated_date_start"))
                .lastUpdatedDateEnd(request.getParameter("last_updated_date_end"))
                .labCode(request.getParameter("lab_code"))
                .hcpName(request.getParameter("hcp_name"))
                .build();
        filterJson = filter.toJsonString();
    }
    if (saveFilter) {
        action.saveLabProceduresFilter(pid, filterJson);
    }
    // Filter the messages
    lpInTransit = action.filterLabProcedures(lpInTransit, filter);
    lpTesting = action.filterLabProcedures(lpTesting, filter);
    lpReceived = action.filterLabProcedures(lpReceived, filter);
        ...
        <table>
            <tr style="text-align: right;">
                <td>
                    ...
                    <input type="text" name="priority" id="priority" value="<%=filter.getPriority() == 0 ? "" : filter.getPriority()%>"/>
                </td>
                <td style="padding-left: 10px; padding-right: 10px;">
                    <label for="lab_procedure_id">Lab Procedure Id:</label>
                    <input type="text" name="lab_procedure_id" id="lab_procedure_id" value="<%=filter.getLabProcedureId() == 0 ? "" : filter.getLabProcedureId()%>"/>
                </td>
                <td>
                    <label for="last_updated_date_start">Start Date: </label>
                    <input type="text" name="last_updated_date_start" id="last_updated_date_start" value="<%=filter.getLastUpdatedDateStart()%>"/>
                    <input type="button" value="Select Date" onclick="displayDatePicker('last_updated_date_start');"/>
                </td>
            </tr>
            <tr style="text-align: right;">
                <td>
                    <label for="lab_code">Lab Code:</label>
                    <input type="text" name="lab_code" id="lab_code" value="<%=filter.getLabCode()%>"/>
                </td>
                <td style="padding-left: 10px; padding-right: 10px;">
                    <label for="hcp_name">HCP Name:</label>
                    <input type="text" name="hcp_name" id="hcp_name" value="<%=filter.getHcpName()%>"/>
                </td>
                <td>
                    <label for="last_updated_date_end">End Date: </label>
                    <input type="text" name="last_updated_date_end" id="last_updated_date_end" value="<%=filter.getLastUpdatedDateEnd()%>"/>
                    <input type="button" value="Select Date" onclick="displayDatePicker('last_updated_date_end');"/>
                </td>
            </tr>
