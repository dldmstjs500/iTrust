...
<form action="viewDiagnosisStatistics.jsp" method="post" id="formMain">
    ...
    <table class="fTable" align="center" id="diagnosisStatisticsSelectionTable">
        ...
        <tr class="subHeader">
            ...
            <td>
                <select name="icdCode" style="font-size:10"
                        onchange="hideThreshold(this.options[this.selectedIndex].value)">
        ...
        <tr>
            <td colspan="4" style="text-align: center;"><input type="submit" id="select_diagnosis"
                                                               value="View Statistics"></td>
        </tr>
    </table>
...
<script>
    ...
    function drawChart() {
        $(function () {
            console.log("Initializing data for graphing");
            var dataTable = google.visualization.arrayToDataTable([
                ["View Type", "Cases in Zip Code", "Cases in Region"],
                ["Current Period", <%=dsBean.getZipStats()%>, <%=dsBean.getRegionStats()%>],
                ["Average\n(Since earliest incident)", <%=avgBean.getZipStats()%>, <%=dsBean.getRegionStats()%>]
            ]);
            var view = new google.visualization.DataView(dataTable);
            var options = {
                height: 300,
                title: "Epidemic Information: " + "<%=icdCode.equals("84.50") ? "Malaria" : "Influenza"%>",
                hAxis: {
                    title: "Average number of diagnoses",
                    minValue: 0
                },
                vAxis: {
                    title: "Period"
            ...
            if (stage.length > 0) {
                ...
                var chart = new google.visualization.BarChart(stage[0]);
...
<h1>
    <center><font color="#FF0000">THERE IS AN EPIDEMIC OCCURRING IN THIS REGION!</font></center>
</h1>

<% } else if (!isEp) {%>

<h1>
    <center>There is no epidemic occurring in the region.</center>
</h1>
