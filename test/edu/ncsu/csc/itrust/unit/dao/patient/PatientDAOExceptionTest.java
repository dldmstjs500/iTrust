...
public class PatientDAOExceptionTest extends TestCase {
	...
	/**
	 * Test that getCauseOfDeath throws a DBException
	 * @throws Exception
	 */
	public void testGetCausesOfDeath() throws Exception {
		try {
			evilDAO.getCausesOfDeath(new Date(100, 1, 1),
					new Date(110, 1, 1));
			fail("DBException should have been thrown");
		} catch (DBException e) {
			assertEquals(EvilDAOFactory.MESSAGE, e.getSQLException().getMessage());
	...
	/**
	 * Test that getCausesOfDeathGender throws a DBException
	 * @throws Exception
	 */
	public void testGetCausesOfDeathGender() throws Exception {
		try {
			evilDAO.getCausesOfDeathGender(new Date(100, 1, 1),
					new Date(110, 1, 1), "Male");
			fail("DBException should have been thrown");
		} catch (DBException e) {
			assertEquals(EvilDAOFactory.MESSAGE, e.getSQLException().getMessage());
		}
