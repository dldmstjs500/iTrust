	...
	public void addObstetricsRecord(ObstetricsRecordBean p, ArrayList<FlagsBean> flags) 
		...
		if (p != null) {
			...
			if (p.getPregnancyStatus().equals(PregnancyStatus.Office)) {
				...
				FlagsBean wFlag = new FlagsBean(p.getMid(),p.getPregId() ,FlagValue.WeightChange);
				...
				FlagsBean fhrFlag = new FlagsBean(p.getMid(),p.getPregId(), FlagValue.AbnormalFHR );
				...
				FlagsBean bpFlag = new FlagsBean(p.getMid(), p.getPregId(),FlagValue.HighBloodPressure);
			...
			//UC67: Allergies Flag			
			FlagsBean allergiesFlag = new FlagsBean(p.getMid(), p.getPregId(),FlagValue.MaternalAllergies );
			...
			if (chosenPatient.getAge() > 35) {
				FlagsBean advancedMaternalAge = new FlagsBean(p.getMid(),p.getPregId(), FlagValue.AdvancedMaternalAge);
			...
			if (chosenPatient.getBloodType().toString().contains("-")) {
				FlagsBean rhNeg = new FlagsBean(p.getMid(), p.getPregId(),FlagValue.rhNeg );
			...
			if (priorMiscarriageCount > 1) {
				FlagsBean miscarriage = new FlagsBean(p.getMid(), p.getPregId(), FlagValue.GeneticMiscarriage);
			...
			if (!conditions.isEmpty()) {
				FlagsBean conds = new FlagsBean(p.getMid(),p.getPregId(), FlagValue.PreExistingConditions );
