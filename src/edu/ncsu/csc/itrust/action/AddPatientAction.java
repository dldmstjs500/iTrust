...
public class AddPatientAction {
	...
	public long addPreRegisteredPatient(PatientBean p, String password) throws FormValidationException, ITrustException {
		new AddPatientValidator().validate(p);
		if(patientDAO.checkForPatientsWithEmail(p.getEmail()))
		{
			throw new FormValidationException("Email");
		}
		else {
			long newMID = patientDAO.addEmptyPatient();
			p.setMID(newMID);
			String pwd = authDAO.addUser(newMID, Role.PATIENT, password);
			p.setPassword(pwd);
			p.setActivated(0);
