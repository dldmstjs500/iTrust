...
public class ViewMyMessagesAction {
     ...
     * Assumes that it is filtering an inbox. Used for backwards compatibility.
     * TODO: Refactor this across the rest of the application.
     *
	...
	public List<MessageBean> filterMessages(List<MessageBean> messages, String filter) throws ITrustException, ParseException {
        return filterMessagesBidir(messages, filter, false);
	}
	/**
	 * Gets a list of messages for a user based on their filter criteria.
     * If filtering an outbox, it will filter names based on the recipient of the message.
     * Otherwise, it will filter names based on the sender.
	 * 
	 * @param messages List of all of a user's MessageBeans
	 * @param filter String containing a user's filter criteria.
	 * @return a List of MessageBeans that meet the criteria of the filter.
	 ...
	 * @throws ParseException
	 */
	public List<MessageBean> filterMessagesBidir(List<MessageBean> messages, String filter, boolean outbox) throws ITrustException, ParseException {
		...
		for(MessageBean m : messages) {
			 ...
			 */
			if(!f[0].equals("")) {
				long nameId = outbox ? m.getTo() : m.getFrom();
				if(!this.getName(nameId).equalsIgnoreCase(f[0]))
					continue;
			}
			/**
	...
	/**
	 * Returns a string showing the primary recipient/sender of the message and
	 * those who were CC'd onto the message.
	 * @param isOutbox 	Whether this message is in the context of the recipient's or sender's mailbox
	 * @param origMessage Original message to get CC'd names for
	 * @return 				String indicating the original sender/primary recipient + all CC'd people
	 * @throws ITrustException
	 * @throws SQLException
	 */
	public String getMessageToString(boolean isOutbox, MessageBean origMessage) throws ITrustException, SQLException {
		String ret = getName( isOutbox ? origMessage.getTo() : origMessage.getFrom());
		List<String> ccNames = getCCdMessages(origMessage.getMessageId())
				.stream()
				.map(msg -> {
					try {
						return getPersonnelName(msg.getTo());
		...
		String ccString = " (CC'd: " + String.join(", ", ccNames) + ")";
		if (ccNames.size() > 0) {
			ret += ccString;
		}
		return ret;
