...
public class ViewDiagnosisStatisticsAction {
	...
	public ArrayList<DiagnosisStatisticsBean> getEpidemicStatistics(String startDate, String icdCode, String zip, String threshold) throws FormValidationException, DBException {
		...
		while( cal.getTime().before(lower) && cal.get(Calendar.YEAR) != lowerCal.get(Calendar.YEAR)) {
            ...
            advanceCalendarToNextYear(cal,weekOfYr);
		}
	...
	/**
	 * Given a specific date, icdCode, and zipcode,
	 * return eight weeks DiagnosisStatistic data of eight weeks before that date
	 * @param startDate
	 * @param icdCode
	 * @param zip
	 * @return
	 ...
	 * @throws DBException
	 */
	public ArrayList<DiagnosisStatisticsBean> getEightWeeksCount(String startDate, String icdCode, String zip) throws FormValidationException, DBException {
		//System.out.println("line312");
		if (startDate == "" || icdCode == "")
			return new ArrayList<>();

		//System.out.println("line319");
		Date upper;  //lower, which is parsed to startDate
		try {
			upper = new SimpleDateFormat("MM/dd/yyyy").parse(startDate);
		} catch (ParseException e) {
			throw new FormValidationException("Enter dates in MM/dd/yyyy");
		}
		if (!zip.matches("([0-9]{5})|([0-9]{5}-[0-9]{4})"))
		...
		ArrayList<DiagnosisStatisticsBean> ret = new ArrayList<DiagnosisStatisticsBean>();
		do{
			ret.add(diagnosesDAO.getCountForWeekOf(icdCode, zip, cal.getTime()));
			cal.add(Calendar.HOUR, -7*24);
			count +=1;
		}while (count<=8);
	...
	private void advanceCalendarToNextYear(Calendar c, int week)
    {
