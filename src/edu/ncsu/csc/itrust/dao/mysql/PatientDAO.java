...
public class PatientDAO {
	...
	public void removePatient(long mid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	public void editPatient(PatientBean p, long hcpid) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("UPDATE patients SET firstName=?,lastName=?,email=?,"
					...
					+ "AlternateName=?, DateOfDeactivation=?, Height=?, Weight=?, Smoker=?, Activated=? WHERE MID=?");

			patientLoader.loadParameters(ps, p);
			ps.setLong(41, p.getMID());
			ps.executeUpdate();
			if(hcpid!=-1) {
	...
	public List<PatientBean> getPreRegisteredPatients() throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM patients Where activated = 0");
			ResultSet rs = ps.executeQuery();
			List<PatientBean> loadlist = patientLoader.loadList(rs);
			rs.close();
			ps.close();
	...
	public boolean checkForPatientsWithEmail(String email) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
	...
	/**
	 * Get the specified patients message filter
	 * @param patientMID patient to find
	 * @return String of message filter
	 * @throws DBException, ITrustException
	 */
	public String getMessageFilter(long patientMID) throws DBException, ITrustException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT MessageFilter FROM patients WHERE MID = ?;");
			ps.setLong(1, patientMID);
			ResultSet results = ps.executeQuery();
			if (results.next()) {
	...
	/**
	 * Saves the patient id's specified message filter
	 * @param filterString String to save
	 * @param pid patient to update
	 * @throws DBException
	 */
	public void saveMessageFilter(String filterString, long pid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
	...
	/**
	 * Get the causes of death of patients in the specified range with the specified gender
	 * @param lower Start date
	 * @param upper End date
	 * @param gender gender of patient
	 * @return strings of the form mid|code|desc
	 * @throws DBException
	 */

	public ArrayList<String> getCausesOfDeathGender(Date lower, Date upper, String gender) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT patients.MID, patients.CauseOfDeath, icdcodes.Description FROM patients INNER JOIN icdcodes ON patients.CauseOfDeath = icdcodes.Code WHERE DateOfDeath >= ? AND DateOfDeath <= ? AND Gender = ?");
			...
			while (rs.next()) {
				...
				tr.add(Long.toString(mid) + "|" + cod + "|" + des);
			}
			return tr;
		} catch (SQLException e) {
			throw new DBException(e);
	...
	/**
	 * Get the causes of death of patients in the specified range
	 * @param lower Start date
	 * @param upper End date
	 * @return strings of the form mid|code|desc
	 * @throws DBException
	 */
	public ArrayList<String> getCausesOfDeath(Date lower, Date upper) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT patients.MID, patients.CauseOfDeath, icdcodes.Description FROM patients INNER JOIN icdcodes ON patients.CauseOfDeath = icdcodes.Code WHERE DateOfDeath >= ? AND DateOfDeath <= ?");
			ps.setTimestamp(1, new Timestamp(lower.getTime()));
			ps.setTimestamp(2, new Timestamp(upper.getTime()));
			ResultSet rs = ps.executeQuery();
