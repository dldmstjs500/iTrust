...
public class PersonnelDAO {
	 ...
	 * 
	 * @param pBean The personnel bean with the updated information.
	 * @throws DBException
	 */
	...
	/**
	 * Get the specified personnel's message filter
	 * @param pid personnel to find
	 * @return String of message filter
	 * @throws DBException, ITrustException
	 */
	public String getMessageFilter(final long pid) throws DBException, ITrustException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT MessageFilter FROM personnel WHERE MID = ?;");
			ps.setLong(1, pid);
	...
	/**
	 * Saves the personnel id's specified message filter
	 * @param filterString String to save
	 * @param pid personnel to update
	 * @throws DBException
	 */
	...
	/**
	 * Get the specified personnel's lab filter
	 * @param pid personnel to find
	 * @return String of lab filter
	 * @throws DBException, ITrustException: DB if something goes wrong, ITrust if user not found
	 */
	public String getLabProceduresFilter(long pid) throws DBException, ITrustException, SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT LabFilter FROM personnel WHERE MID = ?;");
			ps.setLong(1, pid);
			ResultSet results = ps.executeQuery();
			if (results.next()) {
				String result = results.getString(1);
	...
	/**
	 * Saves the personnel id's specified lab filter
	 * @param pid personnel to update
	 * @param filterJson filter to save
	 * @throws DBException
	 */
	public void saveLabProceduresFilter(long pid, String filterJson) throws DBException, SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("UPDATE personnel SET LabFilter = ? WHERE MID = ?");
