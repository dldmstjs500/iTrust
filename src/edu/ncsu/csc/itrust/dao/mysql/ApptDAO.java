...
public class ApptDAO {
	...
	/**
	 * Return all appointments for patients in the following n days
	 * @param n
	 * @return
	 * @throws SQLException
	 * @throws DBException
	 */
	public List<ApptBean> getAllApptForComingDays(int n)throws SQLException, DBException{
		if(n <=0){
			return null;
		}
		Calendar cal = Calendar.getInstance();
		cal.setTime(new Date());
		cal.add(Calendar.HOUR, 24*n);
		...
		try {
			conn = factory.getConnection();
			pstring = conn.prepareStatement("SELECT * FROM appointment WHERE sched_date> NOW() AND sched_date<= ? ORDER by sched_date DESC ");
			//System.out.println(cur.getTime());

			pstring.setTimestamp(1, new Timestamp(upper.getTime()));
