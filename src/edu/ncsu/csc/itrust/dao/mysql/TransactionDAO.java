...
public class TransactionDAO {
	 ...
	 * 
	 * @return List of all transaction log sorted by time logged.
	 * @throws DBException
	 */
	...
	/**
	 * Add and sort roles for user lists that matches with the given MID and return the list.
	 *
	 * @param tbList User list from users table
	 * @param patientID MID of a patient
	 * @param sortByRole If sortByRole is true, this method sorts the list by role.
	 * @return tbList with added role name and sorted list if sortByRole is true.
	 * @throws DBException
	 */
	...
	/**
	 * Returns the list of transaction logs during the given period of date from transactionlog table.
	 *
	 * @param lower The starting date as a java.util.Date
	 ...
	 * @throws DBException
	 */
	public List<TransactionBean> getTransactionsForDate(java.util.Date lower, java.util.Date upper) throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
			ps = conn
					.prepareStatement("SELECT * FROM transactionlog WHERE transactionCode IN ("
							+ TransactionType.patientViewableStr
							+ ") "
							+ "AND timeLogged >= ? AND timeLogged <= ? "
							+ "ORDER BY timeLogged DESC");
			...
			// add 1 day's worth to include the upper
			ps.setTimestamp(2, new Timestamp(upper.getTime() + 1000L * 60L * 60 * 24L));
	 ...
	 * @throws DBException
	 */
	public String getRole(long mid) throws DBException {
		Connection conn = null;
		PreparedStatement ps = null;
		try {
			conn = factory.getConnection();
